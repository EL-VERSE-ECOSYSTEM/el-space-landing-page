        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {trustFeatures.map((feature, idx) => {
              ...
              return (
                <Card key={idx} className="group border-slate-200 bg-white rounded-[2rem] hover:border-transparent hover:shadow-2xl hover:shadow-cyan-500/10 transition-all duration-500 overflow-hidden">
                  <CardHeader className="p-8 pb-4">
                    <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-cyan-50 text-cyan-600 group-hover:scale-110 transition-transform duration-500">
                  ...
                  <CardContent className="p-8 pt-0">
                    ...
                    <Badge className="bg-cyan-100 text-cyan-700 hover:bg-cyan-100 border-none px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{feature.highlight}</Badge>
                  </CardContent>
                </Card>
              );
            })}
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {matchingFeatures.map((feature, idx) => {
              const Icon = feature.icon;
              return (
                <Card key={idx} className="group border-slate-200 bg-white rounded-[2rem] hover:border-transparent hover:shadow-2xl hover:shadow-purple-500/10 transition-all duration-500 overflow-hidden">
                  <CardHeader className="p-8 pb-4">
                    <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-purple-50 text-purple-600 group-hover:scale-110 transition-transform duration-500">
                      <Icon className="w-7 h-7" />
                    </div>
                    <CardTitle className="text-xl font-bold text-slate-900">{feature.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="p-8 pt-0">
                    <p className="text-slate-500 mb-6 leading-relaxed">{feature.description}</p>
                    <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-100 border-none px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{feature.highlight}</Badge>
                  </CardContent>
                </Card>
              );
            })}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-3">
            {paymentFeatures.map((feature, idx) => {
              const Icon = feature.icon;
              return (
                <Card key={idx} className="group border-slate-200 bg-white rounded-[2rem] hover:border-transparent hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 overflow-hidden">
                  <CardHeader className="p-8 pb-4">
                    <div className="mb-6 inline-flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-600 group-hover:scale-110 transition-transform duration-500">
                      <Icon className="w-7 h-7" />
                    </div>
                    <CardTitle className="text-xl font-bold text-slate-900">{feature.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="p-8 pt-0">
                    <p className="text-slate-500 mb-6 leading-relaxed">{feature.description}</p>
                    <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none px-4 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{feature.highlight}</Badge>
                  </CardContent>
                </Card>
              );
            })}
