    ...
    <DashboardLayout navItems={navItems} userType="client">
      <div className="space-y-16 relative z-10 text-foreground">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div>
            <h1 className="text-6xl font-black text-foreground tracking-tighter uppercase">Client <span className="bg-gradient-to-r from-slate-600 to-slate-500 bg-clip-text text-transparent">Cockpit</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
           {[
             { label: 'Vault Assets', value: `$${stats.walletBalance.toLocaleString()}`, icon: DollarSign, color: 'bg-slate-500', iconColor: 'text-slate-500-foreground' },
             { label: 'Active Operations', value: stats.activeProjects, icon: Briefcase, color: 'bg-slate-500/10', iconColor: 'text-slate-500' },
             { label: 'Total Capital Outlay', value: `$${(stats.totalSpent / 1000).toFixed(1)}k`, icon: TrendingUp, color: 'bg-slate-500/10', iconColor: 'text-slate-500' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                 ...
                 <Card key={project.id} className="bg-card border border-border hover:border-slate-500/20 hover:shadow-2xl hover:shadow-success/5 transition-all duration-500 cursor-pointer group rounded-[2.5rem] shadow-md" onClick={() => router.push(`/jobs/${project.id}`)}>
                    <CardContent className="p-8 flex items-center justify-between">
                       <div className="flex items-center gap-8">
                          ...
                          <div>
                             <h4 className="text-foreground font-black text-2xl tracking-tight group-hover:text-slate-500 transition-colors">{project.title}</h4>
          ...
          <div className="lg:col-span-4 space-y-12">
             <Card className="bg-secondary border-none rounded-[3rem] overflow-hidden relative shadow-2xl">
                <div className="absolute top-0 right-0 w-48 h-48 bg-slate-500/10 rounded-full blur-[80px] -mr-24 -mt-24" />
                ...
                <CardContent className="p-10 space-y-10 relative z-10">
                   ...
                   <div className="flex items-center justify-between p-2">
                      <div className="flex items-center gap-4">
                         <div className="w-3 h-3 bg-slate-500 rounded-full animate-pulse shadow-[0_0_15px_rgba(34,197,94,0.5)]" />
