  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-cyan-100 relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-24 -left-24 w-96 h-96 bg-emerald-500/5 rounded-full blur-[100px]" />
        <div className="absolute top-1/2 -right-24 w-[500px] h-[500px] bg-green-500/5 rounded-full blur-[120px]" />
      </div>
      ...
      <header className="h-24 bg-white/70 backdrop-blur-2xl border-b border-slate-100 flex items-center justify-between px-12 sticky top-0 z-50">
        <div className="flex items-center gap-12">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => router.push('/')}>
            <div className="w-10 h-10 rounded-2xl bg-emerald-600 flex items-center justify-center shadow-xl shadow-emerald-200 transition-all group-hover:scale-110">
              <span className="text-white font-black text-sm">EL</span>
            </div>
            <span className="text-xl font-black tracking-tighter text-emerald-900 uppercase">Client</span>
          </div>
          <nav className="hidden md:flex items-center gap-8">
            <button className="text-emerald-900 font-black text-sm border-b-2 border-emerald-900 pb-1">Cockpit</button>
            <button onClick={() => router.push('/jobs')} className="text-slate-400 hover:text-emerald-900 transition-colors font-bold text-sm">Pipeline</button>
            <button onClick={() => router.push('/messages')} className="text-slate-400 hover:text-emerald-900 transition-colors font-bold text-sm">Messages</button>
            <button onClick={() => router.push('/wallet')} className="text-slate-400 hover:text-emerald-900 transition-colors font-bold text-sm">Vault</button>
          </nav>
        </div>

        <div className="flex items-center gap-6">
          <Button onClick={() => router.push('/jobs/post')} className="bg-emerald-600 hover:bg-emerald-700 text-white font-black px-8 h-12 rounded-2xl hidden sm:flex transition-all shadow-lg shadow-emerald-200">
      ...
      <main className="max-w-7xl mx-auto px-12 py-16 space-y-16 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div>
            <h1 className="text-6xl font-black text-slate-900 tracking-tighter">Client <span className="bg-gradient-to-r from-emerald-600 to-green-600 bg-clip-text text-transparent">Cockpit</span></h1>
          ...
          <div className="flex gap-3">
             <Badge className="bg-emerald-50 border-emerald-100 text-emerald-700 px-6 py-2.5 rounded-2xl text-[10px] font-black tracking-[0.2em] uppercase shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8">
           {[
             { label: 'Vault Assets', value: `$${stats.walletBalance.toLocaleString()}`, icon: DollarSign, color: 'bg-emerald-900', text: 'text-white' },
             { label: 'Active Operations', value: stats.activeProjects, icon: Briefcase, color: 'bg-emerald-500', text: 'text-emerald-600' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
                 ...
                 <Card key={project.id} className="bg-white border border-slate-100 hover:border-transparent hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 cursor-pointer group rounded-[2.5rem]" onClick={() => router.push(`/jobs/${project.id}`)}>
                    <CardContent className="p-8 flex items-center justify-between">
                       <div className="flex items-center gap-8">
                          ...
                          <div>
                             <h4 className="text-slate-900 font-black text-2xl tracking-tight group-hover:text-emerald-600 transition-colors">{project.title}</h4>
          ...
          <div className="lg:col-span-4 space-y-12">
             <Card className="bg-emerald-900 rounded-[3rem] overflow-hidden relative shadow-2xl shadow-emerald-100">
                ...
                <CardContent className="p-10 space-y-10 relative z-10">
                   <div className="p-8 bg-white/5 rounded-[2rem] border border-white/5 backdrop-blur-sm">
                      <p className="text-[10px] text-emerald-400 mb-4 font-black uppercase tracking-[0.2em]">Hot Insight</p>
                      ...
                      <Button className="w-full mt-8 h-14 bg-white text-emerald-900 hover:bg-emerald-400 hover:text-white font-black text-sm rounded-2xl transition-all">Review Applications</Button>
                   </div>
             ...
             <div className="space-y-6">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-between bg-white border border-slate-100 hover:border-transparent hover:shadow-2xl hover:shadow-emerald-100 transition-all p-8 rounded-[2.5rem] group"
                >
                   <div className="flex items-center gap-6">
                      <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center group-hover:bg-emerald-50 transition-colors">
                         <Search className="w-6 h-6 text-slate-400 group-hover:text-emerald-600" />
                      </div>
                      <span className="text-slate-900 font-black text-lg tracking-tight">Recruit Talent</span>
                   </div>
                   <ArrowUpRight className="w-6 h-6 text-slate-300 group-hover:text-emerald-900 transition-colors" />
                </button>
